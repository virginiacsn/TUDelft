...
%     'TriggerLevel', 0.1,...
%     'TriggerEdge', 'rising',...
    'TraceVars',[vars.trigger; vars.FZ; vars.TX; vars.TY],...
...
mlib('StartCapture');
clock1 = clock;
for trialNr = 1 : Ntrials
    
    %Laden trial
    trial = protocol.trials(trialNr + ((trialKracht - 1) * protocol.settrials))
    ...
    %Instellen krachtniveau en visueel
    mlib('write', vars.visueel, 'data', trial.visueel);
    mlib('write', vars.Ftarget, 'data', krachtniveau);
    ...
    while vars.trigger < 0.1
        pause(0.1)
    end

    Tijd(trialNr) = clock - clock1;
