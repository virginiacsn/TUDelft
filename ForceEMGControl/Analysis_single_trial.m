...
load([filepath,'TrialData/trial_data']);
...
for h = 4:8
    ...
    for j = 1:length(Aparams.channelName)
        subplot(2,length(Aparams.channelName),j);
        ...
        title(['Musc: ',Aparams.channelName{j}]);
        
    end
    for j = 1:length(Aparams.channelName)
        subplot(2,length(Aparams.channelName),j+length(Aparams.channelName));
