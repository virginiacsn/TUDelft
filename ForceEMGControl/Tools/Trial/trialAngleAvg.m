...
for iangle = 1:length(angles)
    ...
    for ifield = 1:length(fields)
        ...
        trial_data_avg(iangle).(field_str{1}).([field_str{2},'_sem']) = std(field_data_all,1)/size(field_data_all,1);
        trial_data_avg(iangle).(field_str{1}).([field_str{2},'_psem']) = std(field_data_all,1)/length(angle_data);
